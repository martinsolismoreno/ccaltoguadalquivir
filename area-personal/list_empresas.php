...
if (($nivel == GESTOR)OR($nivel == ADMON)) {  //sólo pueden acceder a listados los gestores y el administradore
         ...
         if (!$error) {
		     ...
		     elseif (POST("pagina"))                            listadoFormulario(SESSION("ConEmp")->irPag(POST("pagina")),$campos); //la página se ha pedido por el formulario mostrado
		     ...
		     elseif (array_key_exists("siguiente",$parametros)) listadoFormulario(SESSION("ConEmp")->pagSiguiente(),$campos);
                       else                                             listadoFormulario(SESSION("ConEmp")->actualizarPag(),$campos); //si no se indicó ningún parámetro de pagina, anterior, etc...
