...
if (($nivel == GESTOR)OR($nivel == ADMON)) {  //sólo pueden acceder a listados los gestores y el administradore
         ...
         $error="";
         if (chequearParametros($parametros, $parametrosConsulta, "paramConEmpPte", $error)) {
             $cadOrdenacion = ordenar(SESSION("paramConEmpPte"),$subordenaciones);
             if (!$_SESSION["ConEmpPte"]=Consulta::obtenerConsultaEmpresas($cadOrdenacion,TAM_PAG_EMP,"no")) $error=ETIQ_CONSULTA_FALLIDA;
             $parametros["inicio"]=""; //damos de alta el parámetro "inicio" para que se muestre la primera página si no hay error...
         } else if (!$error) SESSION("ConEmpPte")->actualizarPag();

         if ((!$error) AND (!SESSION("ConEmpPte")->totReg())) $error=ETIQ_NO_HAY_REGISTROS;
         ...
         if (!$error) {
       		         ...
       		         if (array_key_exists("inicio",$parametros))    listadoFormulario(SESSION("ConEmpPte")->pagInicial(),$campos);
		     elseif (array_key_exists("pagina",$parametros))    listadoFormulario(SESSION("ConEmpPte")->irPag($parametros["pagina"]),$campos);
		     elseif (POST("pagina"))                            listadoFormulario(SESSION("ConEmpPte")->irPag(POST("pagina")),$campos); //la página se ha pedido por el formulario mostrado
		     elseif (array_key_exists("final",$parametros))     listadoFormulario(SESSION("ConEmpPte")->pagFinal(),$campos);
                     elseif (array_key_exists("anterior",$parametros))  listadoFormulario(SESSION("ConEmpPte")->pagAnterior(),$campos);
		     elseif (array_key_exists("siguiente",$parametros)) listadoFormulario(SESSION("ConEmpPte")->pagSiguiente(),$campos);
                       else                                             listadoFormulario(SESSION("ConEmpPte")->actualizarPag(),$campos); //si no se indicó ningún parámetro de pagina, anterior, etc...
...
function listadoFormulario($datos, $campos) {
        ...
        $mensajes=array();
        $mensajes[]="Empresas del ".SESSION("ConEmpPte")->posIniPag().
                    " al ".SESSION("ConEmpPte")->posFinPag().
		    " (de ".SESSION("ConEmpPte")->totReg().")";
	$smarty->assign('mensajes',$mensajes);
        ...
        for($c=0; $c<count($campos); $c++) {
            $sentido="asc";
            if (SESSION_CLAVE("paramConEmpPte","orden") == $campos[$c]["bd"]) {
                $sentido = (SESSION_CLAVE("paramConEmpPte","sentido") == "asc") ? "desc" : "asc";
       	    ...
       	    $smarty->assign('link_th'.$c,completarURL(PAG_LIST_PTE."?orden=".$campos[$c]["bd"]."&sentido=".$sentido));
       	    $smarty->assign('dato_th'.$c,$campos[$c]["casilla"]);
       	}
        for($f=0; $f<count($datos); $f++) {
            for($c=0; $c<count($campos); $c++) {
                if ($campos[$c]["bd"] == "empresa") {
                    $smarty->assign('link_td'.$f.$c,'<a href="'.completarURL(PAG_VPREV_EMP."?usuario=".$datos[$f]["idUsuario"]).'">');
                }
      	...
      	$smarty->assign('pagAct',SESSION("ConEmpPte")->pagAct());
 	$smarty->assign('totPag',SESSION("ConEmpPte")->totPag());
	$smarty->assign('totReg',SESSION("ConEmpPte")->totReg());

        $smarty->assign('link_ini',completarURL(PAG_LIST_PTE."?inicio"));
        $smarty->assign('link_ant',completarURL(PAG_LIST_PTE."?anterior"));
        $smarty->assign('link_sgt',completarURL(PAG_LIST_PTE."?siguiente"));
        $smarty->assign('link_fin',completarURL(PAG_LIST_PTE."?final"));

    	$smarty->assign('action',completarURL(PAG_LIST_PTE));
